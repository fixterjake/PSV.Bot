...
class CommandDispatcher {
  ...
  isUserAuthorized(message, matchingCommand, botContext) {
    ...
    const role = guild.roles.find((role) => role.name == matchingCommand.role);
    return matchingCommand.role == 'All' || message.member.roles.has(role.id);
  }
  ...
  reloadCommand(fileNameAndExtension, message = null) {
    ...
    try {
      ...
      this.sendErrorToAuthor(
        new Error(`Couldn't reload ${fileNameAndExtension}: ${error.message}`),
        message
      );
      return false;
    }
